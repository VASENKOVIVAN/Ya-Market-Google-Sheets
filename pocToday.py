...
for campaingId in campaignIdArray:
    ...
    if pagesCount1 > 1:
        for i in range(2, pagesCount1+1):
            response2 = requests.get(
                ...
                '&supplierShipmentDateTo=' + date_today + '&status=PROCESSING',
                headers=HEADERS
            ).json()
    ...
    if today.weekday()+1 == 1:
        ...
        response2 = requests.get(
            ...
            date_yesterday + '&status=PROCESSING',
            headers=HEADERS
        ).json()
        ...
        for i in range(2, pagesCount2+1):
            ...
            if today.weekday()+1 == 1:
                ...
                response2 = requests.get(
                    ...
                    '&supplierShipmentDateTo=' + date_yesterday + '&status=PROCESSING',
                    headers=HEADERS
                ).json()
                data = data + response2['orders']
            else:
